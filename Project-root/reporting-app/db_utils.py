...
def generate_report(filename="report.txt"):
    ...
    with open(filename, "w") as f:
        ...
        # Weekly totals per consultant + customer
        f.write("\nWEEKLY TOTALS (PER CONSULTANT):\n")
        f.write("-" * 80 + "\n")
        for row in weekly_rows:
            consultant, customer, year, week, hours, minutes = row
            f.write(f"Week {int(week):02d}, {int(year)} | {consultant:<15} | {customer:<20} | {int(hours):02d}:{int(minutes):02d}\n")
        ...
        for row in customer_weekly_rows:
            ...
            f.write(f"Week {int(week):02d}, {int(year)} | {customer:<20} | {int(hours):02d}:{int(minutes):02d}\n")
